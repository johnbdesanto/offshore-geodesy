...
def load_pride_positions(files: list[str], time_round: int = constants.DELAY_TIME_PRECISION):
    """
    Loads gps positions into a pandas dataframe from a list of files.
    This assumes input data in a kin file format generated by PRIDE PPP-AR processing.

    The kin file format features a header of variable length that must be skipped.
    Positions are logged in XYZ so may be directly imported. However,
    time records are logged in two columns for modified julian day (MJD)
    and second of day (Sod), so must be converted to J2000 seconds.

    In addition, PRIDE PPP-AR does not calculate position uncertainties for
    kinematic processing. As a temporary fix, we assume a constant
    position uncertainty of 0.01 m in every component.
    ...
    for file in files:
        ...
        with Path(file).open() as f:
            ...
            while line.split()[0] != "END":
                line = f.readline()
                if line == "":
                    msg = "Reached EOF before end of header in \n"
                    msg += f"    {file}.\n"
                    msg += "Please check the file format for the phrase END OF HEADER."
    ...
    # Determine J2000 seconds from MJD and Sod data fields
    all_gnss_positions[constants.TIME_ASTRO] = AstroTime(
        all_gnss_positions["Mjd"], format="mjd", scale="tt"
    )
    all_gnss_positions[constants.TT_TIME] = (
        all_gnss_positions.apply(lambda row: row[constants.TIME_ASTRO].unix_j2000, axis=1)
        + all_gnss_positions["Sod"]
    )

    # Set a dumy value for the position uncertainties
    all_gnss_positions["ant_sigx"] = 0.01
    all_gnss_positions["ant_sigy"] = 0.01
    all_gnss_positions["ant_sigz"] = 0.01
